...
set ForbiddenLink within Links; 
...
sum{(k,i) in Links, o in Origins} Flow1[k,i,o] + sum{(k,i) in Links, o in Origins} Flow2[k,i,o] == Loads[i];

# the two databases aren't connected
subject to ForbiddenLinkConstrain{(i,j) in ForbiddenLink}:
# or: LinkActivation['A','C'] == 0;
Flow1[i,j,'A'] + Flow2[i,j,'A'] == 0;
